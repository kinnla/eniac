	...
	public void setProperties(List<Property> l) {
		for (Property p : l) {
			if (p.getName().equals(Tag.NUMBER)) {
				...
				// it.remove(); ==> need to remove from iterator (== property
// list) ?
			}
		}
	...
	public long getNumber() {
		...
		Switch sign = (Switch) getGarten().getKind(EType.BLINKEN_SIGN_SWITCH, 0);
		long number = sign.getValue();

		// add digit by digit
		EData[] digits = getGarten().getKinder(EType.BLINKEN_NUMBER_SWITCH);
		for (int i = 0; i < 10; ++i) {
			number *= 10;
