...
public class Constant2Lights extends ParentData implements Observer {
	...
	public void init() {
		...
		unit.getGarten().getKind(EType.CONSTANT_SIGN_TOGGLE_JL, 0).addObserver(this);
		unit.getGarten().getKind(EType.CONSTANT_SIGN_TOGGLE_JR, 0).addObserver(this);
		unit.getGarten().getKind(EType.CONSTANT_SIGN_TOGGLE_KL, 0).addObserver(this);
		unit.getGarten().getKind(EType.CONSTANT_SIGN_TOGGLE_KR, 0).addObserver(this);

		// observe constant switchs
		EData[] switchs = unit.getGarten().getKinder(EType.CONSTANT_SWITCH);
	...
	private ConstantTransmitter2 getTransmitter() {
		return (ConstantTransmitter2) getConfiguration().getGarten().getKind(EType.CONSTANT_TRANSMITTER_2_UNIT, 0);
	}
	...
	public void update(Observable o, Object arg) {
		...
		if (t == EType.CONSTANT_SIGN_TOGGLE_JL) {
			Switch sign = (Switch) getGarten().getKind(EType.CONSTANT_BLINKEN_SIGN, 0);
			sign.setValue(((Switch) d).getValue());
		}
		else if (t == EType.CONSTANT_SIGN_TOGGLE_JR) {
			Switch sign = (Switch) getGarten().getKind(EType.CONSTANT_BLINKEN_SIGN, 1);
			sign.setValue(((Switch) d).getValue());
		}
		else if (t == EType.CONSTANT_SIGN_TOGGLE_KL) {
			Switch sign = (Switch) getGarten().getKind(EType.CONSTANT_BLINKEN_SIGN, 2);
			sign.setValue(((Switch) d).getValue());
		}
		else if (t == EType.CONSTANT_SIGN_TOGGLE_KR) {
			Switch sign = (Switch) getGarten().getKind(EType.CONSTANT_BLINKEN_SIGN, 3);
			sign.setValue(((Switch) d).getValue());
		}
		else if (t == EType.CONSTANT_SWITCH) {
			Switch sw = (Switch) getGarten().getKind(EType.CONSTANT_BLINKEN_CIPHER, d.getIndex());
