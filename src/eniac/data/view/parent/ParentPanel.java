...
public class ParentPanel extends EPanel {
    ...
    public ParentPanel() {
        ...
        // init cache
        int l = StringConverter.toInt(EProperties.getInstance().getProperty(
                "BOUNDS_CACHE_SIZE"));
    ...
    public void paintAsIcon(Graphics g, int x, int y, int w, int h, int lod) {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            children[i]
                    .paintAsIcon(g, r.x + x, r.y + y, r.width, r.height, lod);
        }
    }
    ...
    protected Rectangle[] getRectangles(int lod, int width, int height) {
        ...
        if (_boundsCache[index] == null || _widthCache[index] != width
